...
| `SEARCH_RECOMMENDED_ENGINES` | `duckduckgo`&#124;`通用搜索、隐私优先`;`bing`&#124;`图片、新闻`;... | 分号分隔的推荐引擎，可附带 `引擎名`&#124;`说明`。显示在 `info` 输出。留空则不显示。完整示例见下方配置。 |
...
| `info` | `[scope]` | 实例信息。默认：数量、分类、设置。`scope: "engines"` 显示完整引擎列表。 |
